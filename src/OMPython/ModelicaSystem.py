...
class ModelicaSystem:
    def __init__(
            ...
            session: Optional[str | OMCSessionZMQ] = 'create',
        ...
        Args:
            ...
            omhome: OPENMODELICAHOME value to be used when creating the OMC
              session.
            session: OMC session to be used. If unspecified or 'create', a new session
              will be created. Use 'none' for not creating a session.
        ...
        if isinstance(session, str):
            if session == 'create':
                ...
                raise ModelicaSystemError("Invalid session command provided!")
        elif isinstance(session, OMCSessionZMQ):
        ...
        if self.getconn is not None:
            ...
            self.tempdir = pathlib.Path(customBuildDirectory)
        ...
        else:  # if the model is pre-built
            self.xmlFile = self.tempdir / f"{self.modelName}_init.xml"
            print(self.tempdir, self.modelName, self.xmlFile)
            self.xmlparse()
